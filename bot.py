...
class Bot:
    ...
    async def say_with_attachment(
        self, *, channel_id, title, text, is_announcement=False, message_type=None
    ):
        """
    ...
    async def _library_release(self, command_args):
        ...
        await self._wait_for_travis(
            repo_info=repo_info, version=version,
        )
    ...
    async def _wait_for_travis(self, *, repo_info, version):
        ...
        await self.say(
            ...
            text="My evil scheme {version} for {project} has been merged!".format(
                version=version, project=repo_info.name,
            ),
    ...
    async def _web_application_release(self, command_args, hotfix_version=None):
        ...
        await self._wait_for_deploy_rc(repo_info=repo_info,)
        await self.wait_for_checkboxes(
            repo_info=repo_info, manager=command_args.manager,
        )
    ...
    async def wait_for_checkboxes_command(self, command_args):
        ...
        """
        await self.wait_for_checkboxes(
            repo_info=command_args.repo_info, manager=command_args.manager
        )
    ...
    async def commits_since_last_release(self, command_args):
        ...
        async with init_working_dir(
            ...
            has_new_commits = await any_new_commits(
                last_version, base_branch="master", root=working_dir
            )
        ...
        if unchecked_authors:
            ...
            await self.say(
                ...
                "The following authors have not yet checked off their boxes for {project}: {names}".format(
                    names=", ".join(slack_usernames), project=repo_info.name,
                ),
            )
    ...
    async def needs_review(self, command_args):
        ...
        await self.say_with_attachment(
            ...
            title=title,
            text="\n".join(
                "*{repo}*: {title}\n{url}".format(repo=repo, title=title, url=url,)
    ...
    async def issue_release_notes(self, command_args):
        ...
        prs_and_issues = fetch_issues_for_pull_requests(
            github_access_token=self.github_access_token, pull_requests=prs,
        )

        await self.say_with_attachment(
            channel_id=channel_id,
            title=f"Release notes for issues closed by PRs between {start_date} and today",
            text=make_issue_release_notes(
                [pr_and_issue async for pr_and_issue in prs_and_issues]
            ),
            message_type="mrkdwn",
        )
    ...
    async def help(self, command_args):
        ...
        await self.say_with_attachment(
            channel_id=channel_id, title=title, text=text,
        )
    ...
    async def reset(self, command_args):
        ...
        await self.say(
            channel_id=command_args.channel_id,
            text="Um, hello, falling to my doom here!",
        )
    ...
    async def list_tasks(self, command_args):
        ...
        await self.say_with_attachment(
            channel_id=command_args.channel_id,
            title=title,
            text="\n".join(f"{task.task} on {task.channel_id}" for task in self.tasks)
            if self.tasks
            else "No tasks running or scheduled",
        )
                ...
                if len(args) != len(command.parsers):
                    await self.say(
                        ...
                        text="Careful, careful. I expected {expected_num} words but you said {actual_num}.".format(
                            expected_num=len(command.parsers), actual_num=len(args),
                        ),
                    )
                    return
                    ...
                    try:
                        ...
                        await self.say(
                            ...
                            text=(
                                ...
                                " figuring out what that means.".format(word=arg,)
                            ),
                        )
                        return
    ...
    async def handle_message(self, *, manager, channel_id, words):
        ...
        try:
            await self.run_command(
                manager=manager, channel_id=channel_id, words=words,
            )
        ...
        if len(all_words) > 0:
            ...
            if message_handle in ("<@{}>".format(self.doof_id), "@doof"):
                await self.handle_message(
                    manager=message["user"], channel_id=channel_id, words=words,
                )
    ...
    async def startup(self):
        ...
        for repo_info in self.repos_info:
            ...
            self.loop.create_task(
                self.wait_for_checkboxes(
                    manager=None, repo_info=repo_info, speak_initial=False
                )
            )
