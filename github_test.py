...
async def test_karma(mocker):
    ...
    patched.assert_called_once_with(
        github_access_token=github_access_token, query=KARMA_QUERY,
    )
...
def _load_status(status):
    ...
    with open(
        os.path.join(BASE_DIR, "test_data", "statuses_{}.json".format(status))
    ) as f:
        return json.load(f)
...
@pytest.mark.parametrize(
    "status_code,status_data, expected_status",
    [
...
async def test_get_status_of_pr(mocker, status_code, status_data, expected_status):
    ...
    assert (
        await get_status_of_pr(
            github_access_token=token, org=org, repo=repo, branch=branch,
        )
        == expected_status
    )
    ...
    patched.assert_called_once_with(
        mocker.ANY, endpoint, headers=github_auth_headers(token)
    )


async def test_fetch_pull_requests_since_date(
    mocker,
    ...
    result_in_bounds = {
        "data": {
            "organization": {
                "repository": {"pullRequests": {"edges": [edge_in_bounds]}}
            }
        }
    }
    result_out_bounds = {
        "data": {
            "organization": {
                "repository": {"pullRequests": {"edges": [edge_out_bounds]}}
            }
        }
    }
    ...
    prs = [
        pr
        async for pr in fetch_pull_requests_since_date(
...
async def test_fetch_issues_for_pull_requests(mocker):
    ...
    prs_and_issues = [
        tup
        async for tup in fetch_issues_for_pull_requests(
            github_access_token=token, pull_requests=async_gen_wrapper([pr1, pr2]),
        )
    ]
    assert prs_and_issues == [
        ...
        (pr2, [(issues[456], make_parsed_issue(456, False))]),
    ]
    for number in 345, 456:
        patched.assert_any_call(
            github_access_token=token,
...
async def test_make_issue_release_notes():
    ...
    pr1, pr2 = (
        make_pr(1234, "fixes #123 and related to #456"),
        make_pr(3456, "Related to #456"),
    )
    prs_and_issues = [
        (
            pr1,
        ...
        (pr2, [(issue456, make_parsed_issue(456, False))]),
    ]

    assert (
        make_issue_release_notes(prs_and_issues)
        == f"""- {issue123.title} (<{issue123.url}|#{issue123.number}>)
...
async def test_get_issue_but_its_a_pr(mocker):
    ...
    assert (
        await get_issue(
            github_access_token=token, org=TEST_ORG, repo=TEST_REPO, issue_number=1234,
        )
        is None
    )
    patched.assert_called_once_with(
