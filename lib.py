...
def load_repos_info(channel_lookup):
    ...
    infos = [
        RepoInfo(
            ...
            update_other_repos=None  # this is filled in below
    ...
    infos = [
        RepoInfo(
            **{k: v for k, v in info._asdict().items() if k != "update_other_repos"},
            update_other_repos=[
                UpdateOtherRepo(
                    name=other_dict["name"],
                    packaging_tool=other_dict["packaging_tool"],
                    repo_info=repo_info_lookup[other_dict["name"]],
                ) for other_dict in repo_dict_lookup[info.name].get("update_other_repos", [])
            ]
        ) for info in infos
    ]
