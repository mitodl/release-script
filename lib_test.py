...
async def test_get_release_pr(mocker):
    ...
    get_mock = mocker.async_patch(
        ...
        return_value=mocker.Mock(json=mocker.Mock(return_value=FAKE_PULLS)),
    )
    pr = await get_release_pr(github_access_token=access_token, org=org, repo=repo,)
    get_mock.assert_called_once_with(
        mocker.ANY,
        "https://api.github.com/repos/{org}/{repo}/pulls".format(org=org, repo=repo,),
...
async def test_get_release_pr_no_pulls(mocker):
    ...
    mocker.async_patch(
        "client_wrapper.ClientWrapper.get",
        return_value=mocker.Mock(json=mocker.Mock(return_value=[OTHER_PR])),
    )
    assert (
        await get_release_pr(
            github_access_token="access_token", org="org", repo="repo-missing",
        )
        is None
    )
...
async def test_too_many_releases(mocker):
    ...
    mocker.async_patch(
        "client_wrapper.ClientWrapper.get",
        return_value=mocker.Mock(json=mocker.Mock(return_value=pulls)),
    )
    ...
    assert (
        ex.value.args[0]
        == "More than one pull request for the branch release-candidate"
    )
...
async def test_get_unchecked_authors(mocker):
    ...
    get_release_pr_mock = mocker.async_patch(
        ...
        return_value=ReleasePR(
            body=FAKE_RELEASE_PR_BODY, version="1.2.3", url="http://url"
        ),
    )
...
async def test_next_workday_at_10():
    ...
    assert next_workday_at_10(saturday_at_8am) == datetime(
        2017, 4, 3, 10, tzinfo=timezone.utc
    )
    tuesday_at_4am = datetime(2017, 4, 4, 4, tzinfo=timezone.utc)
    assert next_workday_at_10(tuesday_at_4am) == datetime(
        2017, 4, 5, 10, tzinfo=timezone.utc
    )
    wednesday_at_3pm = datetime(2017, 4, 5, 15, tzinfo=timezone.utc)
    assert next_workday_at_10(wednesday_at_3pm) == datetime(
        2017, 4, 6, 10, tzinfo=timezone.utc
    )
...
FAKE_SLACK_USERS = [
    {"profile": {"real_name": "George Schneeloch",}, "id": "U12345",},
...
@pytest.mark.parametrize(
    "testing,python2,python3", list(product([True, False], repeat=3))
)
async def test_upload_to_pypi(
    testing, python2, python3, mocker, library_test_repo, test_repo_directory
):
    ...
    library_test_repo = RepoInfo(
        **{**library_test_repo._asdict(), "python2": python2, "python3": python3,}
    )
    ...
    def check_call_func(*args, env, **kwargs):
        ...
        if command_args[0].endswith("twine"):
            ...
            assert env["TWINE_USERNAME"] == (
                twine_env["PYPITEST_USERNAME"]
                if testing
                else twine_env["PYPI_USERNAME"]
            )
            assert env["TWINE_PASSWORD"] == (
                twine_env["PYPITEST_PASSWORD"]
                if testing
                else twine_env["PYPI_PASSWORD"]
            )
    ...
    init_working_dir_mock.assert_called_once_with(
        token, library_test_repo.repo_url, branch=f"v{version}"
    )
...
async def test_load_repos_info(mocker):
    ...
    assert load_repos_info({"bootcamp-eng": "bootcamp_channel_id"}) == [
        RepoInfo(
            name="bootcamp-ecommerce",
            repo_url="https://github.com/mitodl/bootcamp-ecommerce.git",
            rc_hash_url="https://bootcamp-ecommerce-rc.herokuapp.com/static/hash.txt",
