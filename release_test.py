...
async def test_init_working_dir_real():
    ...
    repo_url = "https://github.com/mitodl/release-script.git"
    access_token = ""
    async with init_working_dir(access_token, repo_url,) as other_directory:
        assert os.path.exists(other_directory)
...
async def test_create_release_notes(test_repo_directory, with_checkboxes):
    ...
    if with_checkboxes:
        ...
        assert_starts_with(lines, ["- Commit #3", "- Commit #2", "- Commit #1", "",])
...
async def test_release(mocker, hotfix_hash, test_repo_directory):
    ...
    update_version_mock.assert_called_once_with(
        new_version, working_dir=test_repo_directory
    )
